...
program define nl_dld_fe, rclass
    ...
    local params b1Cb b2Cb b3Cb b1Ca b2Ca b3Ca b1Tb b2Tb b3Tb b1Ta b2Ta b3Ta L
    foreach p of local params {
        capture confirm local `p'
        if _rc == 0 local `p'_val = ``p''
        else local `p'_val = 0
    }

    gen double `term1' = .
    replace `term1' = ((1 - exp(-`L_val'*``Maturity'')) / (`L_val'*``Maturity'')) ///
        if `touse' & `L_val'!=0 & ``Maturity''>0
    replace `term1' = 1 if `touse' & (`L_val'==0 | ``Maturity''==0)

    gen double `term2' = .
    replace `term2' = (`term1' - exp(-`L_val'*``Maturity'')) if `touse'

    gen double `f_it' = ///
        (`b1Cb_val') + (`b2Cb_val')*`term1' + (`b3Cb_val')*`term2' + ///
        (`b1Ca_val')*``Post'' + (`b2Ca_val')*``Post''*`term1' + (`b3Ca_val')*``Post''*`term2' + ///
        (`b1Tb_val')*``Treated'' + (`b2Tb_val')*``Treated''*`term1' + (`b3Tb_val')*``Treated''*`term2' + ///
        (`b1Ta_val')*``DiD'' + (`b2Ta_val')*``DiD''*`term1' + (`b3Ta_val')*``DiD''*`term2'
